...
- 🟡 **Asistente IA (Gemini, gratis) en "¿Algo no funciona?"** (24-sep, fase 2): tras enviar el reporte, `functions/api/asistente-soporte.js` responde en vivo (streaming); WhatsApp al equipo solo si la persona toca «Necesito al equipo» o se va sin resolver. Bandeja con «Analizar con IA» (diagnóstico + respuesta sugerida). Rediseño con paleta de marca completa + solo Inter (ESTANDARES-UX-TIPOGRAFIA.md). SQL `sql/reportes-web-ia-2026.sql` corrido. Usa la `GEMINI_API_KEY` que ya existe (Claude opcional con `IA_PROVEEDOR=claude` + `ANTHROPIC_API_KEY`, de pago). Portado gemelo a Alejandro CAD/CAM.
