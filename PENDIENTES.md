...
|---|--------|-------|---------|
| 0j | **Ejecutar `sql/patch-revoke-rpcs-internas.sql`** | Supabase PRODIGY → SQL Editor | RPCs `prodigy_marcar_recordatorio`, `prodigy_marcar_sla_alerta`, `prodigy_set_sla` (uso exclusivo de crons via service_role) tenían GRANT a `authenticated` — cualquier doctor logueado podía silenciar alertas SLA/pago de cualquier pedido o cambiar el SLA objetivo de un flujo completo. Revoca `authenticated`. |
...
| 0m | **Corregir HSTS en Cloudflare** → SSL/TLS → Edge Certificates → "HTTP Strict Transport Security (HSTS)" → Max Age Header: cambiar a 6 meses o más (≥2592000) | Cloudflare Dashboard (zona prodigylabdental.com) | **Auditoría SecurityScorecard**: aunque `_headers` define `max-age=63072000`, el header real servido en producción es `Strict-Transport-Security: max-age=0; includeSubDomains; preload` — la config de zona de Cloudflare está sobrescribiendo `_headers` con max-age=0. Esto le quita prácticamente todo el valor a HSTS (capa la nota en "A" en securityheaders.com) y es contradictorio con `preload`. |
| 0n | **Configurar DMARC + verificar dominio Resend en Cloudflare DNS** | Cloudflare Dashboard → DNS | **Auditoría SecurityScorecard — DNS Health**: `_dmarc.prodigylabdental.com` NO existe (sin política anti-spoofing/reporting). Además `resend._domainkey.prodigylabdental.com` y `send.prodigylabdental.com` tampoco existen → Resend probablemente NO está verificado para este dominio, por lo que los correos enviados desde `noreply@/bienvenida@/alertas@/sistema@prodigylabdental.com` (vía `functions/api/*.js`) pueden fallar DKIM/SPF y caer en spam. Acción: (1) en Resend Dashboard → Domains → agregar `prodigylabdental.com`, copiar los 3 registros DNS que entrega (MX bounce, SPF TXT, DKIM TXT `resend._domainkey`) a Cloudflare; (2) agregar TXT `_dmarc` con `v=DMARC1; p=quarantine; rua=mailto:gerencia@prodigylabdental.com; pct=100` (subir a `p=reject` tras confirmar alineación). `PENDIENTES-DNS-EMAIL.md` está desactualizado (asume Google Workspace; el MX real es Cloudflare Email Routing) — corregir tras aplicar esto. |
