...
- **⚠️ AUDIT 2026-09-07 — NO es un swap de URL; hay riesgo de MALA ENTREGA:**
...
3. ✅ **RLS 42501 — CAUSA RAÍZ DEFINITIVA, corregida en código.** El insert anónimo SÍ está permitido (verificado: POST con `Prefer: return=minimal` → 201). El problema era el `.insert([...]).select('id')`: ese `.select()` agrega un `RETURNING id` que necesita una policy **SELECT para anon** sobre la fila recién creada; como no existe (la única SELECT anon filtra por token de revisión), PostgREST **revierte el INSERT entero** y devuelve 42501. Los 4 diagnósticos (columnas, enum, policies, permissive/restrictive) descartaron todo lo demás; el trigger `prodigy_forzar_estado_inicial_pedido` no toca columnas de identidad. **Fix (sin SQL nuevo):** el flujo ahora **genera el `id` (uuid) client-side** con `crypto.randomUUID()` y lo manda en el insert, eliminando el `.select('id')`/RETURNING. Así el insert pasa y el flujo conserva el id para registrar archivos. Aplicado en los 5 flujos (PRODIGY x4 + Alejandro). Verificado con `node --check`.
