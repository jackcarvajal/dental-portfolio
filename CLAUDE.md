...
- `tools/` — **auditoría automática (correr antes de push/deploy)**: `audit.mjs` (estático: anon key, IDs dup, links rotos), `audit-live.mjs` (runtime: errores JS, 4xx Supabase), `audit-schema.mjs` (contrato front↔BD vs CSV), `audit-schema-live.mjs` (contrato front↔BD **contra la base REAL**: columnas/RPCs inexistentes, sin snapshot). El pre-push corre `audit.mjs` + `audit-schema-live.mjs`. Ver `tools/README.md`.
